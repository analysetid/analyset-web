...
10. **Setiap penambahan page atau perubahan page**, pastikan sitemap & statusnya ter-update di Google Search Console: (a) sitemap Astro auto-regenerate saat build, (b) submit ulang sitemap via GSC API (`/opt/data/.venv_gws/bin/python3 /opt/data/scripts/gsc/gsc_api.py sitemaps submit --site sc-domain:analyset.com --feedpath https://analyset.com/sitemap-index.xml`; token di `/opt/data/.hermes/gsc_token.json`), (c) inspect URL baru/berubah via `gsc_api.py inspect --url <url>`; bila perlu Request Indexing manual di UI.
